...
- `tools.ts` — `read_section`, `list_suggestions`, `list_attachments`, `search_documents`, `document_outline`, `read_document_page`, `ask_user`, draft/edit tools, `insert_image` (`chat` / `section` / `analytics`), `plot_measurements` (on for every pack; omit only when embedding Document tools in Analytics chat); sanitizes untrusted metadata here (not in `src/lib/attachments/`). `draft_field` / `propose_edit` / `edit_table` run `groundDraftText` against the citation ledger's served quotes. Recap / assessment narratives and title-page / user-confirmed identity (`citation-exemption.ts`) skip that gate; inventory tables stay strict. `finish_document_review` seeds `reviewedEvidence` (every walked page) so the gate is not limited to the 60-finding sample. Empty ledger or pages with no quotes fail open. Pack `unsupportedFactPolicy`: MJ `block` (invented hard facts do not persist; leftover `<date>`/`<identifier>`/`<number>` persist after the repair lookup when those facts are still missing), demo/Convergent `flag`. Analytics `write_column` is not gated.
- `claim-facts.ts` / `evidence-match.ts` / `ground-draft.ts` / `citation-exemption.ts` / `citation-grounding.ts` / `unsupported-facts-repair.ts` — hard-fact extraction, uFuzzy page matching, misattributed cite rewrite, `unsupported_facts` tool result. Skip / frame modes omit title-page identity, Indian FY bounds, and recap/assessment prose from the gate (inventory tables stay strict). Parked `[n]` resolves through the Citations: list. High-collision dates keep the cited file/page (empty quote or many hits) instead of stealing the first SOP/PRQ page; identifier mismatches still move. A shared parked `[n]` gets a new list line rather than overwriting the other claim. Table grounding passes sibling cells (documentRef) into that ranking. A blocked MJ write runs one closed-set repair search and re-grounds; leftover placeholders persist after that lookup when the facts are still missing. Suggestion cards persist `claimProvenance` and show Traceability. Audit `claim_verified` / `claim_unsupported`.
...
**Chat retrieval:** `searchReportDocuments`, `readDocumentPage`, `readDocumentOutline` in `src/lib/attachments/retrieval.ts`. Identifier queries (`requirementIds()`) hit `document_pages.identifiers` first and skip the query embedding when those hits fill `limit`. Results collapse to one chunk per page. FTS: `to_tsvector('english', contextual_text)` + `document_chunks_contextual_text_fts_en_idx`. Outline reads prefer stored `document_outline_spans`. Plan: `docs/retrieval.md`. Ingest pipeline: `docs/document-ingest-pipeline.md`. Junior-engineer briefing (diagrams): write path `docs/harness-briefing/03-indexing-and-storage.pdf`; read path `docs/harness-briefing/04-retrieval-and-grounding.pdf`. Release gates: `docs/pdf-evidence-deployment-checklist.md`.
