...
5. `Stat → Scatter…` (or analytics `plot_xy_scatter`) plots a worksheet scatter (`xy_scatter`). Y is required and must be numeric. X is optional: omit it for Y vs observation index (1, 2, 3…); pass a numeric X for Y vs X. Optional legend column color-codes points (`layout.seriesBy: "unit"`); labels/lots/serials are valid legend values but cannot be X. Pair rows where the required axes parse as numbers; Pearson r is overall (null if n<2 or zero variance). Y-column LSL/USL draw as horizontal dashed lines. `plot_measurements` remains attachment series vs observation index (one series, one color).
