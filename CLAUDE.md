...
- `document-review.ts` — `selectReviewPages` fair-shares pages. `start_document_review` returns `truncated` / `skippedDocuments`. Rehydrate `coverageKey` includes the review objective so a finished walk of one section does not skip another.
