...
- `src/app/admin/` + `src/app/api/admin/` — Admin console (audit log viewer, user management, retention/password-policy settings, Limits, Usage). API: `audit`, `users` (+ `reset-password`, `unlock`), `password-policy`, `retention`, `ai-budget`, `usage` (time-in-app + DAU/WAU/MAU), `attachment-page-budget`, `attachment-storage-budget`, `voice-budget`, `reports/[reportId]/{purge,source-docx}`.
