...
- `claim-facts.ts` / `evidence-match.ts` / `ground-draft.ts` / `citation-grounding.ts` — hard-fact extraction, uFuzzy page matching, misattributed cite rewrite, `unsupported_facts` tool result. Suggestion cards persist `claimProvenance` and show Traceability. Audit `claim_verified` / `claim_unsupported`.
