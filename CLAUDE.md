...
pnpm build-fir-template   # rebuild the MJ Investigation Report DS Word template
pnpm extract-fir-chrome   # re-extract DS page chrome from a real SOP/QA/017-F01 report
pnpm preview-fir-docx     # fill the DS template with sample content for a visual (PDF) check
...
- `tools.ts` — `read_section`, `list_suggestions`, `list_attachments`, `search_documents`, `document_outline`, `read_document_page`, `ask_user`, draft/edit tools, `insert_image` (`chat` / `section` / `analytics`), `plot_measurements` (on for every pack; omit only when embedding Document tools in Analytics chat); sanitizes untrusted metadata here (not in `src/lib/attachments/`). `draft_field` / `propose_edit` / `edit_table` run `groundDraftText` against the citation ledger's served quotes. Frame mode exempts title-page / user-confirmed identity, 1 April–31 March bounds, and facts already in this report (`citation-exemption.ts`); copied attachment facts still need a page in every section. Tables stay strict. `finish_document_review` seeds `reviewedEvidence` (every walked page) so the gate is not limited to the 60-finding sample. Empty ledger or pages with no quotes fail open. Pack `unsupportedFactPolicy`: MJ `block` (invented hard facts do not persist; prose leftover `<date>`/`<identifier>`/`<number>` persist after the repair lookup; table leftovers bounce once so a subsequent search can fill them), demo/Convergent `flag`. Analytics `write_column` is not gated.
...
- `claim-facts.ts` / `evidence-match.ts` / `ground-draft.ts` / `citation-exemption.ts` / `citation-grounding.ts` / `unsupported-facts-repair.ts` — hard-fact extraction, uFuzzy page matching, misattributed cite rewrite, `unsupported_facts` tool result. Frame mode omits title-page identity, 1 April–31 March bounds, and facts already stated in this report; tables stay strict. Do not skip a whole section. Parked `[n]` resolves through the Citations: list. High-collision dates keep the cited file/page (empty quote or many hits) instead of stealing the first SOP/PRQ page; identifier mismatches still move. Uncited numbers do not attach a coincidental ledger page (unique identifier pin and unique same-file hit still retarget). Integer tokens do not match inside decimals (`3` is not `3.2 mL`). A shared parked `[n]` gets a new list line rather than overwriting the other claim. Table grounding passes sibling cells (documentRef) into that ranking. A blocked MJ write runs one closed-set repair search and re-grounds; prose leftover placeholders persist after that lookup when the facts are still missing; table leftovers bounce once (`keepSearchOpen`) and may persist on a same-turn retry. Suggestion cards persist `claimProvenance` and show Traceability. Audit `claim_verified` / `claim_unsupported`.
