...
- `claim-facts.ts` / `evidence-match.ts` / `ground-draft.ts` / `citation-grounding.ts` / `unsupported-facts-repair.ts` — hard-fact extraction, uFuzzy page matching, misattributed cite rewrite, `unsupported_facts` tool result. A blocked MJ write runs one closed-set repair search and re-grounds; leftover placeholders persist after that lookup when the facts are still missing. Suggestion cards persist `claimProvenance` and show Traceability. Audit `claim_verified` / `claim_unsupported`.
