...
2. Enter, paste, or ask the assistant to extract values from attachments (`extract_numeric_series` / scan / page reads, then one `write_column` per destination sheet). Pull every page of that table first — an incomplete dump is not saved. Separate extracts for separate sheets are correct; always pass `sheetId`. New series fill the leftmost empty C1–C8 columns (add_column without `at` does the same — it does not append on the right while those placeholders are empty). Extraction also fills column specs when pages name limits.
