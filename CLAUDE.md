...
**Chat:** Tools are search/outline/page/extract/worksheet/sixpack/ANOVA/scatter. Images and Quick/Deep pace match Document chat. No `propose_edit` / `draft_field` / Plan-Agent toggle. Do not add these tools to the report Plan-mode allowlist. Stub chat is text-only (`buildStubAnalyticsChatModel`).
