...
- `repair-tool-call.ts` / `clamp-tool-input.ts` — last-resort tool-call repair. A hallucinated tool name is remapped; an `InvalidToolInputError` is clamped to the tool's own JSON Schema (maxItems / maxLength / min / max / enum) and retried once, so an out-of-bounds argument cannot end the turn. Malformed JSON is not rewritten.
