...
**Citation links:** Clicking `[filename, p. N]` (or numbered `[n]` that maps to a parked source) in the document, chat, or suggestion Sources opens or reuses that attachment's canvas tab and scrolls the PDF to the cited page (`openDocument(id, page)` via `openCitedDocumentOrToast`). Word preview cannot jump to a page. Missing or ambiguous filenames toast instead of opening. Decorations stay out of saved TipTap JSON.
