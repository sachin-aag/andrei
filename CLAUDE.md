...
7. `Plot → Boxplot…` (or analytics `plot_boxplot`) draws a Tukey box-and-whisker of numeric Y. Optional category columns (innermost first, closest to the boxes; last is the outermost nested axis label) group **observed combinations only** (not a full factorial). Zero categories is one box of all Y. Empty category cells are labeled `(blank)`. At most 4 category columns and 80 groups. Agent Analytics chat can create a boxplot or edit an existing one (`plot_boxplot` with `analysisId`, including `showMeanLine`). Ask mode cannot. Whiskers are last observations inside Q1−1.5 IQR / Q3+1.5 IQR; outliers are asterisks. Optional **Show mean line** (default off, not in `sourceHash`) connects each box’s mean; the median line inside the box stays. Time series is not supported.
