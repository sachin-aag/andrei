...
**Failure telemetry:** Every assistant failure is captured to PostHog as the `ai_chat_failed` event plus an exception, keyed by workspace user id. Server sites (`stream_start`, `stream_error` in both the report and analytics routes) go through `captureChatAssistantFailure` (`src/lib/ai/chat/chat-failure-telemetry.ts`) on the shared `posthog-node` client (`src/lib/analytics/posthog-server.ts`). Client sites (`client_error`, `empty_turn` in `chat-session-host.tsx`) use `posthog-js`. A `surface` (`report` / `analytics`) and `site` property tag each event. Never let capture throw — it must not mask the original failure.
