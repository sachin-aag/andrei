...
- `user-intent.ts` — latest-turn classifier (`social` / `read` / `write`). Greetings strip tools; empty sections are not a draft request. Same classifier on Analytics chat. A leading affirmation is stripped and the remainder classified, so "yes put it in the data worksheet" is a write (and is a write outright when the prior assistant turn offered to write). Analytics also treats a worksheet/sheet/column destination as a write. `intentToolAvailabilityRule` puts the stripped tool names in the prompt so a read turn cannot call a tool that is not loaded. `resolve-user-intent.ts` runs Flash-Lite for `ambiguous_agent_mode` (plan/outline vs write), Document-chat worksheet-dump phrasing, and Agent remaining-report writes (whether to seed `pending_plan`). High-confidence `preferredSurface=analytics` shows a Switch to Analytics button that resends the original request on Analytics. The same call sets `sectionQueue` for a multi-section draft; timeout/stub fall back to the remaining-section regex. A yes / go for it after that copy is a write. Do not fold retrieval policy into that call.
...
- `pending-plan.ts` — remaining-section queue on `chat_sessions.pending_plan`. Flash-Lite `sectionQueue` (same intent call) seeds empty `draftOrder` items; regex is the timeout/stub fallback. Each Agent turn drafts the current item (two adjacent non-inventory sections may share a turn). Persist stamps `continuation`; the client POSTs `CHAT_AUTO_CONTINUE_TEXT` with `autoContinue`. Cancel (`pauseReason: cancelled`) and a new typed prompt pause the queue. Auto-continue user rows render as progress chips.
