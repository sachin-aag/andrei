...
| shows Data sheets and column specs from the header menu | Data tab only; insert/delete row/column are not in Data; right-click column Specs dialog filled from sample assay; Plot menu is Sixpack, ANOVA, and worksheet Plot measurements (no attachment extract); attachment plots are Analytics chat |
