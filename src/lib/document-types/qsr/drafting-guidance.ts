...
- Qualification Documents: one row per document (Document Name, Document Number, Revision, Status, Effective/Approved date, Remarks). Start each equipment group with a row that has only the equipment ID (e.g. "PC-1301") in bold in the first cell and every other cell empty. For a protocol/report pair (DQ, IQ, OQ, PQ) put the protocol on the first row and the report number on the next row with Document Name, Revision and Remarks left empty — export merges them.
...
- Requirement Traceability Matrix (5.1–5.6): one row per URS ID from the approved URS; Reference – Qualification Stage is DQ/IQ/OQ/PQ, Reference – Section is the protocol section that verified it. A group row (e.g. "ANY SPECIFIC REQUIREMENTS", "PRIMARY CONDENSER (PC-1301)") has only bold text in the first cell.
