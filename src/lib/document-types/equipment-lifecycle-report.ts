...
- Monitoring is one row per environmental method (non-viable, viable air, settle plate, surface/glove, differential pressure, LAF). Merged methods or SCADA process alarms in that table are not_met.
...
const PER_SECTION_PROMPTS: Record<string, string> = {
  ...
  elr_monitoring: `The assessment must interpret excursion counts and linked deviations, and say whether product or the environment was affected. The table is one row per environmental method (not merged viable methods, not SCADA N2/air alarms). Period Covered is the Indian FY (1 April–31 March), not an alarm-trend quarter.`,
