...
- For unknown facts use angle-bracket placeholders: <Label> (same as the editor). Square brackets are citations ([filename, p. N] or [n]), not fill-in tokens. Do NOT write [Label: <to be filled>] or wrap a document id as [PRQR-25-PR-005: <to be filled>].
- Assume the author will fill existing placeholders later. Treat them as standing in for the labeled fact — do NOT replace <Label> or legacy [Label: <to be filled>] with invented concrete text (e.g. do not change <Procedure reference> to a specific document number).
...
- Guidance-only tokens like <batch number> are OK when inserting new missing text; do not overwrite existing placeholders.
...
- If the section already has a <Label> (or legacy [Label: <to be filled>]) for a missing fact, do not add another placeholder for the same fact.
...
Replace:
{
  ...
  "insertText": "<detection date> at approximately <time> hrs",
