  ...
  z.object({
    ...
    rows: z
      .array(
        z.object({
          row: z.number().int().min(0),
          expectedCells: z.array(z.string()),
        })
      )
      .min(1),
  }),
  ...
  const tools: ToolSet = {
    ...
    edit_table: tool({
      ...
      execute: async ({
        ...
        const applied = applyTableOperation(fieldDoc, parsedOp, {
          section,
          targetField: resolvedField,
        });
        if (!applied.ok) {
        ...
        await db.insert(comments).values({
          ...
          content: serializeAiFixCommentContent({
            ...
            insertText: "",
            reasoning,
            tableOperation: parsedOp,
            contentHashAtSuggestion: sectionContentHash(section, loaded.content),
          }),
          anchorText: summarizeTableOperation(parsedOp),
          contentPath: resolvedField,
          fromPos: null,
          toPos: null,
          status: "open",
    ...
    draft_field: tool({
      description:
        `Draft or fully rewrite ONE field. Provide the COMPLETE replacement content as markdown: paragraphs, '- ' bullets, '1. ' numbered lists, '## ' headings, '**bold**', and GFM tables ('| a | b |' rows with a '| --- |' separator). Use bracketed placeholders like [batch number] for facts you do not know — never invent facts. The engineer reviews the full draft and accepts or rejects it. Use this for empty fields, substantial rewrites, creating a NEW table, or an explicitly requested full table replacement. For any incremental change to an existing table, use edit_table — never draft_field. Use propose_edit only for small prose or list edits.${scopeHint}${fixedTableHint}`,
      inputSchema: z.object({
        section: z.enum(sectionEnum),
        targetField: z
          .string()
