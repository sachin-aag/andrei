...
export const CHAT_PROMPT_VERSION = "chat-v141-qsr-decimals";
...
- One review per section this turn. After finish_document_review returns status complete with coverageComplete true, do not call start_document_review again — not with a rephrased objective, another attachment, or "checking citations". If truncated and skippedDocuments include the PRQR while queued pages were CSV-OQ / RTM / URS headers, start again so those pages are queued; that finish does not unlock edit_table. Do not restart monitoring or breakdowns because the alarm-trend PDF was skipped — Alarm Trends already owns that file. Fill the empty inventory from a real walk. search_documents is for later fact checks, not a second walk.
- On ELR monitoring, one row per Grade A method (non-viable, active viable air, settle plate, surface and glove, differential pressure, LAF). Do not merge methods. Do not queue the alarm-trend PDF — Alarm Trends sits above this section. Read that section and cite [[table:Alarm Trends]] for this period's alarm picture. Do not copy process-alarm codes into the monitoring table. Period Covered is 1 April to 31 March of the following year (both dates), never an alarm-trend quarter.
