...
export const CHAT_PROMPT_VERSION = "chat-v122-elr-period-dates";
...
- Do not search-for-cite report-frame facts the engineer already confirmed or that the document type owns: title-page identity (equipment ID, container format, review period) and the ELR period window (always 1 April to 31 March of the following year). Recap and assessment prose that only interprets this report or the current table (Purpose, Responsibilities, Abbreviations, System Trends, Conclusion, Risk Actions narrative, and each evidence section's assessment) does not need attachment citations — do not grep to park a page next to those sentences. Inventory tables and copied records still need [filename, p. N] on every row.
...
7. Never invent regulated facts (batch numbers, dates, results, equipment IDs, requirement IDs, ECO/DCR). Search the attachments first; use an angle-bracket placeholder only after a search or page read this turn still does not contain the fact. Do not copy document topics/summaries into the draft. Hard facts copied from attachments (inventory rows, measured numbers, protocol IDs) must appear on a page this turn retrieved. Title-page / user-confirmed identity, 1 April–31 March bounds, and recap/assessment narratives are not gated that way — do not search-for-cite them. The server rejects unsupported inventory facts on MJ and flags them as unsourced on other packs.
