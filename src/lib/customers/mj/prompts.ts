/**
 * MJ evaluation prompts (SOP/DP/QA/008). Distinct from the shared demo prompt
 * so MJ and demo eval caches never mix.
 *
 * Bump `MJ_PROMPT_VERSION` when these strings change in a way that should
 * invalidate previously cached MJ evaluations.
 */
export const MJ_PROMPT_VERSION = "mj-sop-dp-qa-008-v1";

export const MJ_EVALUATION_SYSTEM_PROMPT = `You are a pharmaceutical quality assurance reviewer at M.J. Biopharm Private Limited. You evaluate deviation investigation reports written per SOP/DP/QA/008 using a traffic light system:
...
const MJ_DEFINE_PROMPT_ADDITION = `SECTION ROLE - DEFINE:
...
const MJ_MEASURE_PROMPT_ADDITION = `SECTION ROLE - MEASURE:
...
const MJ_ANALYZE_PROMPT_ADDITION = `SECTION ROLE - ANALYZE:
...
const MJ_IMPROVE_PROMPT_ADDITION = `SECTION ROLE - IMPROVE:
...
const MJ_CONTROL_PROMPT_ADDITION = `SECTION ROLE - CONTROL:
...
export const MJ_SECTION_PROMPT_ADDITIONS: Readonly<Record<string, string>> = {
  define: MJ_DEFINE_PROMPT_ADDITION,
  measure: MJ_MEASURE_PROMPT_ADDITION,
  analyze: MJ_ANALYZE_PROMPT_ADDITION,
  improve: MJ_IMPROVE_PROMPT_ADDITION,
  control: MJ_CONTROL_PROMPT_ADDITION,
};
