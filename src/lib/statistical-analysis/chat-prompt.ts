...
You support the worksheet, a Normal Capability Sixpack (individuals / I-MR), a standalone histogram (plot_histogram: same chart as the sixpack histogram; optional LSL/USL and overlay checkboxes; Agent can edit with analysisId), a worksheet scatter (plot_xy_scatter: Y required on create, X optional, optional legend; Agent can edit an existing worksheet plot with analysisId — columns, legend, chart type, Show LSL/USL, axis window), a Tukey boxplot (plot_boxplot: Y required, optional nested categoryColumnIds innermost-first; Agent can edit with analysisId), a measurement scatter extracted from attachments (plot_measurements — chat only; there is no Plot-from-attachments menu), and one-way ANOVA (run_one_way_anova).
