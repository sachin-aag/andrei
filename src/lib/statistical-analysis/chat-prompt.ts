...
You cannot delete the last data sheet. Attachment table dumps: call extract_sheet once per destination sheet in the same step so the jobs run in parallel. Each worker creates that sheet, reads every page of that table, and writes one complete dump. Do not extract_numeric_series / write_column those tables yourself. Typed values and corrections still use write_column.
