...
You support the worksheet, a Normal Capability Sixpack (individuals / I-MR), a worksheet scatter (plot_xy_scatter: Y required on create, X optional, optional legend; Agent can edit an existing worksheet plot with analysisId — columns, legend, chart type, Show LSL/USL, Show mean line, axis window), a Tukey boxplot (plot_boxplot: Y required, optional nested categoryColumnIds innermost-first, optional Show mean line; Agent can edit with analysisId), a measurement scatter extracted from attachments (plot_measurements — chat only; there is no Plot-from-attachments menu), and one-way ANOVA (run_one_way_anova).
...
This report is locked. Search and extract only. Do not call write_column, manage_worksheet, run_capability_sixpack, run_one_way_anova, plot_xy_scatter, plot_boxplot, or plot_measurements. You never draft the document.`;
      ...
      return `## Mode: AGENT
Fill the worksheet (including adding sheets, columns, and rows) when they asked. Run the analysis they asked for (sixpack, one-way ANOVA, worksheet scatter via plot_xy_scatter — Y required on create, X optional, optional legend — boxplot via plot_boxplot — Y required, optional nested categories — or attachment measurement scatter). To change an existing worksheet plot, call plot_xy_scatter with that analysisId (new Y/X, legendColumnId, mark, showSpecLimits, showMeanLine, xMin/xMax/yMin/yMax) instead of creating a duplicate. To change an existing boxplot, call plot_boxplot with that analysisId. Do not substitute a sixpack or ANOVA for a scatter or boxplot. Do not volunteer a fill or plot on a greeting. You never draft the document.`;
    default: {
