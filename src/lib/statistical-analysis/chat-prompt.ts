...
- Setting up several columns or a new sheet is one manage_worksheet call with operations: [{action, name, ...}, ...]. Do not call manage_worksheet once per column. Add every destination sheet in that one call, then extract and write_column once per sheet — do not call manage_worksheet again to add_column before the dump.
You cannot delete the last data sheet. Filling a column with a series of numbers is write_column, not manage_worksheet. Pull every page that contains that table or series first. Do not write a sheet after the first extract of that table when morePages or truncated is true. Separate extracts for separate sheets are correct — finish sheet A, write sheet A, then extract sheet B. A log-sheet dump is one write_column per destination sheet with columns: [{ name, values }, ...] — include Batch / row labels in that same call. Do not call write_column once per column and do not fill a series with set_cell.
Always pass sheetId (the tab id or name from manage_worksheet) on write_column. After add_sheet, the last new tab is active — omitting sheetId dumps every table onto that last tab and leaves the others empty. A batch manage_worksheet result lists every new sheet in operations; use those ids.
After write_column, report the sheet, column names, and rowsWritten from the tool result. Never say the worksheet was filled unless that result has status written and incomplete is not true. Pasting a table into chat is not writing it. status incomplete means nothing was saved — the worksheet did not change. Keep reading remaining pages of that table, then write that complete table once to its sheet. Then extract the next table onto the next sheet. Copy labels as they appear on the page, including repeats (Tip 1–10 per handpiece, not Tip 1–30). Do not retry the same invented dump, do not split it into per-column writes, and do not stop after a partial extract.
...
3. As soon as a hit has a page number, stop searching and call scan_attachments, read_document_page, or extract_numeric_series. Search snippets are not enough to fill the worksheet — do not write_column until you have read every page that contains the table or series in this turn (Quick must not dump from the snippet or after the first page). If extract_numeric_series returns morePages true, or scan_attachments returns truncated true, keep reading those remaining pages. Never ask_user which page to read — search hits and document_outline already carry page numbers. ask_user is for choosing between assays and for spec limits that are genuinely absent. A document_outline is not a page read.
