...
export const ANALYTICS_CHAT_PROMPT_VERSION =
  "analytics-chat-v39-never-ask-page";
...
3. As soon as a hit has a page number, stop searching and call scan_attachments, read_document_page, or extract_numeric_series. Search snippets are not enough to fill the worksheet — do not write_column until you have read or scanned that page in this turn (Quick must not dump from the snippet). Never ask_user which page to read — search hits and document_outline already carry page numbers. If you cannot find the named table after search/scan, say you did not find it. ask_user is for choosing between assays and for spec limits that are genuinely absent from the files. A document_outline is not a page read.
