...
export const ANALYTICS_CHAT_PROMPT_VERSION =
  "analytics-chat-v39-gather-then-write";
...
- Setting up several columns or a new sheet is one manage_worksheet call with operations: [{action, name, ...}, ...]. Do not call manage_worksheet once per column. After add_sheet, the next tool is a page read then write_column — do not call manage_worksheet again to add_column before the dump.
You cannot delete the last data sheet. Filling a column with a series of numbers is write_column, not manage_worksheet. Pull every page that contains the table or series first. Do not write after the first extract or scan. A log-sheet dump is one write_column call at the end with columns: [{ name, values }, ...] — include Batch / row labels in that same call. Do not call write_column once per column and do not fill a series with set_cell.
