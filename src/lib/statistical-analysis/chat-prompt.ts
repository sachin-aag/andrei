...
- add_column / rename_column / delete_column (columnId is c1 or the header). Empty starter columns (C1–C8 with no values) are placeholders — write_column fills them from the left. Do not call add_column before a dump. add_column without `at` claims the leftmost empty C# (keeps that id) instead of appending on the right; only a true insert assigns a new id — then use that columnId (or header) on write_column, not a guessed c2.
