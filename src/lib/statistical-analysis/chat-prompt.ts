...
- How many attachments, which files, folders, or file status: call list_attachments. Do not guess from the Ready documents index. Do not search for an inventory — that greps page text.
...
File-set questions (how many files, names, folders, ready vs still ingesting): call list_attachments. Do not guess from the index. search_documents greps page text and is not a file inventory.
...
function documentIndex(documents: ReadyDocumentIndexItem[]): string {
  if (documents.length === 0) {
    return "Ready documents: none uploaded (or still ingesting). Call list_attachments for the Attachments tree (including files still ingesting).";
  }
  return [
    "Ready documents (index only — call list_attachments for counts, names, folders, or status):",
    ...documents.map((doc) => {
      const filename =
        quotePromptMetadata(
