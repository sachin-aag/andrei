...
it does not reuse Document chat's grep-loop copy. TOC / running-header snippets that only list many requirement IDs are ranked last (`requirementIndex`) and a TOC-only grep retries excluding those pages. Document chat does not treat attachment cover sheets (`divider=true`) as a cited data page — search stays open so the following page can be read. Analytics still stops after a real cited page.
