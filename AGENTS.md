...
of the file list) hides files and folders without removing them from reports. Report workspace
...
overall/within fits share the dense measurement grid with the bars (not one
vertex per bin) so Excel strokes a smooth bell.
