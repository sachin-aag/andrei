...
- Hard facts (dates, identifiers, measured numbers) in `draft_field` / `propose_edit` / `edit_table` must match a quote on a page this turn retrieved (`CitationPageLedger` + `groundDraftText`). Empty ledger or pages with no served quotes fail open. Pack policy: MJ `block` (do not persist; return `unsupported_facts`); demo/Convergent `flag` (persist + Traceability). Analytics `write_column` is not gated. Suggestion cards show Traceability. Audit `claim_verified` / `claim_unsupported`.
