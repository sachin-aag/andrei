...
of the file list) hides files and folders without removing them from reports. A vault scan or upload shows a spinner on Files and in the browser tab. Report workspace
...
- Clicking a citation in the document, chat, or suggestion Sources opens or reuses that file's canvas tab and scrolls the PDF to the cited page (Word preview cannot jump to a page). Combined `[file A, p. N, file B, p. M]` is two links (brackets and the comma stay plain); extra pages of one file stay one link. Missing or ambiguous filenames toast instead of opening a tab. This is an in-app tab, not a browser tab. Open attachment tabs stay mounted while you switch to Report or another file (no “Loading preview…” on return; the PDF stays on the page you left). Closing the tab drops the document and a later open may load again, restored to that last page.
