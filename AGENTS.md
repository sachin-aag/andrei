...
- Clicking a citation in the document, chat, or suggestion Sources opens or reuses that file's canvas tab and scrolls the PDF to the cited page (Word preview cannot jump to a page). Missing or ambiguous filenames toast instead of opening a tab. This is an in-app tab, not a browser tab.
