...
pnpm retrieval-eval:upload                # optional: write PDFs to the test bucket
pnpm retrieval-eval -- --from-gcs         # CI path: download (or seed if empty), ingest, search, judge
...
`--from-gcs` downloads the corpus when it is already in the bucket. If the
objects are missing, CI generates the same PDFs from `retrieval-corpus.ts`
and tries to seed the bucket. That needs `roles/storage.objectCreator` (or
`objectAdmin`) on the eval bucket for the GitHub Actions SA; with
`objectViewer` only, the job still ingests the generated bytes. Laptop
`pnpm retrieval-eval:upload` remains available if you want to seed from ADC.
