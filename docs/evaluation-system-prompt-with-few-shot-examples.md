# Archived evaluation system prompt (few-shot examples)

This file preserves the **previous** `src/lib/ai/section-prompts.ts` from git **before** inline weak/strong `<example>` pairs were removed in favor of criteria-only scoring and leaner prompts.

| | |
| --- | --- |
| **Source path** | `src/lib/ai/section-prompts.ts` |
| **Last commit with this content** | `d978bdce11b153f252c636bb561df8d24ffcf346` (`b2679d2^`, parent of the removal commit) |
| **Removal / replacement commit** | `b2679d2974246634289c8bfdbeb5f752facb180b` — *Align evaluation prompts with criteria-only scoring and Improve DOCX parity.* |
| **Embedded `PROMPT_VERSION`** | `2026-05-17-1` |

To re-fetch this blob yourself:

```bash
git show 'b2679d2^:src/lib/ai/section-prompts.ts'
```

---

## Full archived module (verbatim TypeScript)
