...
- [ ] One attachments bucket per **new** tenant via `infra/gcs` (`tenants` map). Create the bucket as part of [Add a customer](./whitelabel-vercel-deploy.md#add-a-customer) — never put a new pack on `andrei-493614-attachments`. MJ/demo/Convergent still share that legacy bucket; 3xper is `andrei-493614-3xper-attachments`. Retrieval-eval is CI-only.
- [ ] Lifecycle rule deletes **only** `staging/` and `temp/` prefixes (not permanent evidence)
- [ ] Exact CORS origins for browser resumable uploads (app production + preview URLs). Custom domains must be listed exactly on **that tenant’s** bucket — `https://mj.andreihealth.com` is on the shared bucket; `https://3xper.andreihealth.com` is on `andrei-493614-3xper-attachments`. Apply with Terraform (`infra/gcs` `tenants.*.cors_origins`).
- [ ] Optional `ATTACHMENT_PREVIEW_DIRECT_STORAGE=true` — serves PDF preview bytes straight from GCS instead of proxying them through the content route. Requires the origin to be on that bucket’s CORS with `GET`/`HEAD` and `Content-Length` + `ETag` exposed (Terraform already sets those headers). Leave unset until that CORS config is applied to the bucket in that environment, or preview fails with a CORS error and falls back to nothing. Biggest win on 100 MB+ attachments, where the function is otherwise in the path for every byte.
